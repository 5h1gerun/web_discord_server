...
```
`COOKIE_SECRET` を指定しない場合は、`COOKIE_SECRET_FILE` で示すパスにキーが自動生成され、次回以降も再利用されます。
...
OAuth 認証時の `state` はセッションと専用 Cookie `dst` に保存され、コールバック後に検証されます。Drive ファイル一覧取得に失敗した際はサーバー側のエラーメッセージが画面に表示されます。
